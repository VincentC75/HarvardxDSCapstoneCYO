...
However extracting the month or the week could give a better predictor
...
Some variables contain a high number of missing variables. 

```{r}
sapply(rain, function(x) round(sum(is.na(x)) / length(x) * 100))
```

for some of them, the percentage of missing values is so high that trying to impute those values is useless, therefore we keep only those with up to 10% of missing values and eliminate the others: Evaporation, Sunshine, Cloud9am, Cloud3pm
...
For the remaining numerical variables, we will impute the mean of the rest of the observations

```{r}
rain[is.na(rain$WindGustSpeed),]$WindGustSpeed <- round(mean(rain$WindGustSpeed, na.rm = TRUE))
rain[is.na(rain$WindSpeed9am),]$WindSpeed9am <- round(mean(rain$WindSpeed9am, na.rm = TRUE))
rain[is.na(rain$WindSpeed3pm),]$WindSpeed3pm <- round(mean(rain$WindSpeed3pm, na.rm = TRUE))
rain[is.na(rain$Humidity9am),]$Humidity9am <- round(mean(rain$Humidity9am, na.rm = TRUE))
rain[is.na(rain$Humidity3pm),]$Humidity3pm <- round(mean(rain$Humidity3pm, na.rm = TRUE))
rain[is.na(rain$Rainfall),]$Rainfall <- round(mean(rain$Rainfall, na.rm = TRUE), digits = 1)
rain[is.na(rain$Pressure9am),]$Pressure9am <- round(mean(rain$Pressure9am, na.rm = TRUE))
rain[is.na(rain$Pressure3pm),]$Pressure3pm <- round(mean(rain$Pressure3pm, na.rm = TRUE))
rain[is.na(rain$MinTemp),]$MinTemp <- round(mean(rain$MinTemp, na.rm = TRUE), digits = 1)
rain[is.na(rain$MaxTemp),]$MaxTemp <- round(mean(rain$MaxTemp, na.rm = TRUE), digits = 1)
rain[is.na(rain$Temp9am),]$Temp9am <- round(mean(rain$Temp9am, na.rm = TRUE), digits = 1)
rain[is.na(rain$Temp3pm),]$Temp3pm <- round(mean(rain$Temp3pm, na.rm = TRUE), digits = 1)
```

The 3 remaining variables containing missing values are categorical and all related to Wind. Since WindGustDir, WindSpeed9am and WindSpeed3p seem to often have the same values, we try imputation by assigning non missing values from the other two variables

```{r}
rain[is.na(rain$WindGustDir),]$WindGustDir <- rain[is.na(rain$WindGustDir),]$WindDir9am
rain[is.na(rain$WindGustDir),]$WindGustDir <- rain[is.na(rain$WindGustDir),]$WindDir3pm
rain[is.na(rain$WindDir9am),]$WindDir9am <- rain[is.na(rain$WindDir9am),]$WindDir3pm
rain[is.na(rain$WindDir9am),]$WindDir9am <- rain[is.na(rain$WindDir9am),]$WindGustDir
rain[is.na(rain$WindDir3pm),]$WindDir3pm <- rain[is.na(rain$WindDir3pm),]$WindDir9am
rain[is.na(rain$WindDir3pm),]$WindDir3pm <- rain[is.na(rain$WindDir3pm),]$WindGustDir
```

For observations where all three Wind values where missing, we impute the mode, the most common value.

```{r}
rain[is.na(rain$WindDir9am),]$WindDir9am <- names(which.max(table(rain$WindDir9am)))
rain[is.na(rain$WindDir3pm),]$WindDir3pm <- names(which.max(table(rain$WindDir3pm)))
rain[is.na(rain$WindGustDir),]$WindGustDir <- names(which.max(table(rain$WindGustDir)))
```

At this stage, we no longer have missing values in the dataset, and we still have 142193 observations. If we had simply ignored all observations with missing values, we would only have 56420 left. This simple imputation allows us to consider almost three times more observations and thus keeping more signal in the dataset, hopefully leading to more insight.
More elaborate methods for imputation exist and could also be tried to compare results.
...
## Corelation of explanatory variables

The explanotory variables contains different measures of the same quantity. For example we have 4 measures of temperature: min, max, at 9am and at 6pm. As can be expected, the is a relatively strong correlation between these measures.
...
```{r}
pairs(select(rain, contains("Speed")))
```

```{r}
pairs(select(rain, contains("Humidity")))
```

```{r}
pairs(select(rain, c('WindSpeed9am', 'Pressure9am', 'Temp9am')))
```
```{r, message = FALSE}
library(corrplot)
corrplot(cor(select(rain, c('WindSpeed9am', 'Pressure9am', 'Temp9am'))), method = 'number')
```
...
library(caret)
set.seed(1971)
rain <- na.omit(rain) # to delete after imputation
...
TODO : Method to select predictors
Cross validation to select predictors

```{r}
mod_glm <- glm(RainTomorrow ~ WindGustSpeed + Humidity3pm + Pressure3pm, data = train_set, family = binomial)
summary(mod_glm)
```
```{r}
pred_glm <- predict(mod_glm, type = 'response', newdata = test_set)
pred_glm <- as.factor(ifelse (pred_glm < 0.5, "No", "Yes"))
confusionMatrix(data = pred_glm, reference = test_set$RainTomorrow)
```
...
#pred_rf <- as.factor(ifelse (pred_rf < 0.5, "No", "Yes"))
#confusionMatrix(data = pred_rf, reference = test_set$RainTomorrow)
```

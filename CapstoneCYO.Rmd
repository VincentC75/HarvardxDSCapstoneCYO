...
Some predictors are highly correlated. For instance, the most obvious corelations are between Temp9am and MinTemp, Temp 3pm and MaxTemp. Pressure9am and Pressure3pm also seem to bring the same information whith a near perfect corelation of 0.96.
...
TODO : Method to select predictors, based on explanatory analysis above
...
We see that all of the selected predictors are highly significant.
...
```{r}
pred_tree <- predict(mod_tree, newdata = test_set)
#pred_tree2 <- as.factor(pred_tree)
#confusionMatrix(data = pred_tree2, reference = test_set$RainTomorrow, positive = "Yes")
```
